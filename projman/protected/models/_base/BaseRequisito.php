<?php

/**
 * This is the model base class for the table "requisito".
 * DO NOT MODIFY THIS FILE! It is automatically generated by AweCrud.
 * If any changes are necessary, you must set or override the required
 * property or method in class "Requisito".
 *
 * Columns in table "requisito" available as properties of the model,
 * followed by relations of table "requisito" available as properties of the model.
 *
 * @property integer $id
 * @property integer $projecto
 * @property string $descricao
 * @property integer $estado
 *
 * @property AlteracaoRequisito[] $alteracaoRequisitos
 * @property Projecto $projecto0
 * @property Estado $estado0
 * @property RequisitoStakeholder[] $requisitoStakeholders
 */
abstract class BaseRequisito extends AweActiveRecord {

    public static function model($className=__CLASS__) {
        return parent::model($className);
    }

    public function tableName() {
        return 'requisito';
    }

    public static function representingColumn() {
        return 'descricao';
    }

    public function rules() {
        return array(
            array('projecto, descricao, estado', 'required'),
            array('projecto, estado', 'numerical', 'integerOnly'=>true),
            array('descricao', 'length', 'max'=>100),
            array('id, projecto, descricao, estado', 'safe', 'on'=>'search'),
        );
    }

    public function relations() {
        return array(
            'alteracaoRequisitos' => array(self::HAS_MANY, 'AlteracaoRequisito', 'requisito'),
            'projecto0' => array(self::BELONGS_TO, 'Projecto', 'projecto'),
            'estado0' => array(self::BELONGS_TO, 'Estado', 'estado'),
            'requisitoStakeholders' => array(self::HAS_MANY, 'RequisitoStakeholder', 'requisito'),
        );
    }

    /**
     * @return array customized attribute labels (name=>label)
     */
    public function attributeLabels() {
        return array(
                'id' => Yii::t('app', 'ID'),
                'projecto' => Yii::t('app', 'Projecto'),
                'descricao' => Yii::t('app', 'Descricao'),
                'estado' => Yii::t('app', 'Estado'),
                'alteracaoRequisitos' => null,
                'projecto0' => null,
                'estado0' => null,
                'requisitoStakeholders' => null,
        );
    }

    public function search() {
        $criteria = new CDbCriteria;

        $criteria->compare('id', $this->id);
        $criteria->compare('projecto', $this->projecto);
        $criteria->compare('descricao', $this->descricao, true);
        $criteria->compare('estado', $this->estado);

        return new CActiveDataProvider($this, array(
            'criteria' => $criteria,
        ));
    }
    
    public function search_Estado($parentID) {
        $criteria = new CDbCriteria;

        $criteria->compare('id', $this->id);
        $criteria->compare('projecto', $this->projecto);
        $criteria->compare('descricao', $this->descricao, true);
        $criteria->compare('estado', $parentID);

        return new CActiveDataProvider($this, array(
            'criteria' => $criteria,
        ));
    }
    
    public function search_Projecto($parentID) {
        $criteria = new CDbCriteria;

        $criteria->compare('id', $this->id);
        $criteria->compare('projecto', $parentID);
        $criteria->compare('descricao', $this->descricao, true);
        $criteria->compare('estado', $this->estado);

        return new CActiveDataProvider($this, array(
            'criteria' => $criteria,
        ));
    }

    public function behaviors() {
        return array_merge(array(
        ), parent::behaviors());
    }
}